...
const sendMessage = async () => {
  ...
  try {
    ...
    await addDoc(
      collection(db, "chats", senderUid, "chats", recipientUid, "messages"),
      payload
    );
    // write message to recipient's conversation (so they see it under their own nested path)
    await addDoc(
      collection(db, "chats", recipientUid, "chats", senderUid, "messages"),
      payload
    );
    ...
    await setDoc(
      doc(db, "chats", senderUid, "chats", recipientUid),
      convoData,
      { merge: true }
    );
    await setDoc(
      doc(db, "chats", recipientUid, "chats", senderUid),
      convoData,
      { merge: true }
    );
  } catch (err) {
