...
const loadTransactions = async () => {
  console.log('Loading transactions for userId:', props.userId, 'userType:', props.userType);
  ...
  try {
    ...
    if (props.userType === 'parent') {
      q = query(
        paymentsRef,
        where('parentId', '==', props.userId)
      );
    } else {
      q = query(
        paymentsRef,
        where('tutorId', '==', props.userId)
      );
    }
...
const downloadCSV = () => {
    ...
    const headers = ['Transaction ID', 'Date', 'Assignment', 'Amount', 'Status', props.userType === 'parent' ? 'Tutor' : 'Parent'];
  ...
  a.href = url;
  a.download = `transactions_${props.userType}_${new Date().toISOString().split('T')[0]}.csv`;
  a.click();
      ...
      <div class="card-body">
        ...
        <div v-show="isExpanded">
          <p class="text-muted mb-4">
            {{ userType === 'parent' ? 'Payments made to tutors' : 'Payments received from parents' }}
          </p>
          ...
          <div class="row g-3 mb-4">
            ...
            <div class="col-md-4">
              <div class="stat-card">
                ...
                <div class="stat-content">
                  <div class="stat-label">Total {{ userType === 'parent' ? 'Spent' : 'Earned' }}</div>
          ...
          <div v-else-if="error" class="alert alert-danger">
            ...
            {{ error }}
            <button class="btn btn-sm btn-danger ms-2" @click="loadTransactions">
              Retry
            </button>
          </div>
            ...
            <table class="table table-hover">
              ...
              <tbody>
                <tr v-for="transaction in paginatedTransactions" :key="transaction.id">
                    ...
                    <div class="d-flex align-items-center">
                      <div>
                        <div class="fw-semibold">
                          {{ userType === 'parent' ? transaction.tutorName : transaction.parentName || 'N/A' }}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="fw-semibold">{{ formatCurrency(transaction.amount) }}</td>
                  <td>
                    <span 
                      class="badge"
